...
class EnrollmentRepository:
    ...
    def getById(self, enrollment_id):
    ...
    def update(self, enrollment_id, status):
        ...
        enrollment = self.getById(enrollment_id)
        enrollment.status = status

        return enrollment.save()

    @staticmethod
    def create(nanodegree_key, udacity_user_key, status):
        """ Create a new enrollment """
        enrollment = Enrollment(nanodegree_key = nanodegree_key, 
                    udacity_user_key = udacity_user_key, status = status)
